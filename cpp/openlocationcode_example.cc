...
int main() {
  ...
  const openlocationcode::CodeArea code_area = openlocationcode::Decode(code);
  ...
  const bool isValid = openlocationcode::IsValid(code);
  std::cout << "Is valid? " << isValid << std::endl;
  // => true

  // Checks if a Plus+Code is full.
  const bool isFull = openlocationcode::IsFull(code);
  std::cout << "Is full? " << isFull << std::endl;
  // => true

  // Checks if a Plus+Code is short.
  const bool isShort = openlocationcode::IsShort(code);
  ...
  const std::string recovered_code =
